...
Ordinary same- or cross-realm native Promise callback results are consumed and diagnosed as asynchronous. Callback producers remain responsible for suspicious thenables, species behavior, altered Promise shapes, and pre-existing rejections that Kuery cannot safely consume. Untrusted AST/JSON input takes a separate structural path: Promise and other non-plain objects are rejected without async detection or intentionally invoking accessors. This boundary accepts decoded JSON or a trusted plain-data graph; it is not a sandbox for hostile proxies or callback-produced objects.
...
`generateExpressionJsonSchema(profile)` derives strict Draft 2020-12 operator alternatives from the supplied profile's public names and exact/minimum/maximum arity. Node objects are closed, arguments are capped at 32, and unsafe or oversized property names are denied. The schema validates recursive shape and profile constraints for plain JSON literals and the default string-reference codec. Aggregate `maxNodes` and `maxDepth` enforcement is runtime-authoritative during canonicalization and compilation. Executable values and accessors are impossible in JSON; custom reference codecs require a host-specific schema adaptation and remain runtime-validated.
