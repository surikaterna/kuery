...
Ordinary same- or cross-realm native Promise results, including rejections, are consumed and reported as `EXPRESSION_ASYNC_UNSUPPORTED`. Callback producers remain responsible for suspicious thenables, species behavior, altered Promise shapes, and any pre-existing rejection that Kuery cannot safely consume. Untrusted expression AST and JSON values never invoke this async detector: non-plain objects, including Promises, are rejected structurally without intentionally invoking accessors.

`getStandardExpressionJsonSchema()` returns the strict `standard-v1` Draft 2020-12 schema; `generateExpressionJsonSchema(profile)` derives the equivalent schema from a custom profile's operator names and arities. Schemas validate recursive JSON shape and profile constraints using closed node objects, a maximum of 32 operator arguments, bounded property names, and the default string reference representation. Aggregate `maxNodes` and `maxDepth` limits remain authoritative runtime checks during canonicalization and compilation; custom reference codecs remain a runtime boundary and require host-owned schema adaptation.
